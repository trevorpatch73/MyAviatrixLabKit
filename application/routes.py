...
def homepage():
    ...
    if environment is None:
        ...
        db.session.add(entry)
        db.session.commit()
    elif user.db_aws_key_id is not None:
        if user.db_aws_key_value is not None:
            if user.db_terraform_org_name is not None:
                if user.db_terraform_api_key is not None:
